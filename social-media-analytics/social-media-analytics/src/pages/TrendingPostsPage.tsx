import React, { useEffect, useState } from "react";
import { getUsers, getUserPosts, getPostComments } from "../api";
import { Post } from "../types";
import PostCard from "../components/PostCard";
import { Box, Typography, CircularProgress } from "@mui/material";

const TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..TKgnj4XhKPQDRM25ot5gefF6No3tDgNSzWxB6__Hd5M";

const TrendingPostsPage: React.FC = () => {
  const [trendingPosts, setTrendingPosts] = useState<{ post: Post; commentCount: number }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const usersObj = await getUsers(TOKEN);
      const userIds = Object.keys(usersObj);
      let allPosts: Post[] = [];
      for (const userId of userIds) {
        const posts = await getUserPosts(userId, TOKEN);
        allPosts = allPosts.concat(posts);
      }
      const postCommentCounts: { [key: number]: number } = {};
      for (const post of allPosts) {
        const comments = await getPostComments(post.id, TOKEN);
        postCommentCounts[post.id] = comments.length;
      }
      const maxComments = Math.max(...Object.values(postCommentCounts));
      const trending = allPosts
        .filter(post => postCommentCounts[post.id] === maxComments)
        .map(post => ({ post, commentCount: postCommentCounts[post.id] }));
      setTrendingPosts(trending);
      setLoading(false);
    };
    fetchData();
  }, []);

  return (
    <Box p={2}>
      <Typography variant="h4" mb={2}>Trending Posts</Typography>
      {loading ? <CircularProgress /> : trendingPosts.map(({ post, commentCount }) => (
        <PostCard key={post.id} post={post} commentCount={commentCount} />
      ))}
    </Box>
  );
};

export default TrendingPostsPage; 